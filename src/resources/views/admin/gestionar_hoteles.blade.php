                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-primary" 
                                            data-bs-toggle="modal" data-bs-target="#editHotelModal{{ $hotel->id_hotel }}">
                                        <i class="fas fa-edit"></i> Editar
                                    </button>
                                    <button type="button" class="btn btn-sm btn-danger" 
                                            data-bs-toggle="modal" data-bs-target="#deleteHotelModal{{ $hotel->id_hotel }}">
                                        ...
                                        <form action="{{ route('admin.hoteles.actualizar', $hotel->id_hotel) }}" method="POST">
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="Usuario{{ $hotel->id_hotel }}" 
                                                    ...
                                                    <select class="form-select" id="id_zona{{ $hotel->id_hotel }}" name="id_zona" required>
                                                        @foreach($zonas as $zona)
                                                            <option value="{{ $zona->id_zona }}" 
                                                                {{ $hotel->id_zona == $zona->id_zona ? 'selected' : '' }}>
                                                                {{ $zona->descripcion }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="number" class="form-control" id="Comision{{ $hotel->id_hotel }}" 
                                                           name="Comision" value="{{ $hotel->Comision }}" required min="0" max="100">
                                                </div>
                                                <div class="mb-3">
                                                    <label for="password{{ $hotel->id_hotel }}" class="form-label">Contraseña (dejar en blanco para no cambiar)</label>
                                                    <input type="password" class="form-control" id="password{{ $hotel->id_hotel }}" 
                                                           name="password">
                                                </div>
                                            </div>
